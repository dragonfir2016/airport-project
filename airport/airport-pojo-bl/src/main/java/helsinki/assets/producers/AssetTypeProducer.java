package helsinki.assets.producers;
...
public class AssetTypeProducer extends DefaultEntityProducerWithContext<AssetType> {
    ...
    protected AssetType provideDefaultValuesForStandardNew(final AssetType entityIn, final EntityNewAction masterEntity) {
        final AssetType newAssetType = super.provideDefaultValuesForStandardNew(entityIn, masterEntity);
        ...
        if (ofMasterEntity().keyOfMasterEntityInstanceOf(AssetClass.class)) {
            ...
            newAssetType.setAssetClass(refetch(shallowAssetClass, "assetClass"));
            newAssetType.getProperty("assetClass").validationResult().ifFailure(Result::throwRuntime);
            newAssetType.getProperty("assetClass").setEditable(false);
        }
        return newAssetType;
    }
}
